<div class="block md:flex md:space-x-2 px-2 lg:p-0 w-full">
    <a class="mb-4 md:mb-0 w-full relative h-[24em]" href="{{ route('post',$post->slug) }}">
        ...
        @if(!$post->getMedia('posts')->isEmpty())
            <img src="{{ $post->getFirstMediaUrl('posts') }}" class="absolute left-0 top-0 w-full h-full shadow-md rounded-[2rem] @if($loop->odd) ltr:rounded-br-none rtl:rounded-bl-none @else ltr:rounded-bl-none rtl:rounded-br-none @endif z-0 object-cover"/>
        @endif
